...
/* Global Variables */
extern unsigned char stock_data[5], product_data[5];
...
void init_can(void)
{
	...
	ECANCON = 0x00;  //enhanced can control register
...
//	RXF0EIDL = 0x00;
	...
	RXB0CON = 0x00;//receiving control register
}
...
void can_transmit(void)
{
	...
	TXB0DLC = 0x08;			/* Data Length Count - 8 bytes*/
	TXB0D0 = stock_data[0];			/* DataByte 0 */
	TXB0D1 = stock_data[1];			/* DataByte 1 */
	TXB0D2 = stock_data[2];			/* DataByte 2 */
	TXB0D3 = stock_data[3];			/* DataByte 3 */
	TXB0D4 = product_data[0];			/* DataByte 4 */
	TXB0D5 = product_data[1];			/* DataByte 5 */
	TXB0D6 = product_data[2];			/* DataByte 6 */
	TXB0D7 = product_data[3];			/* DataByte 7 */

	TXB0REQ = 1;			/* Set the buffer to transmit - transmission request register*/
}
